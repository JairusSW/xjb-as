...
const edgeF = [
    ...
    6.62607e-34,
    43210.1,
    // irregular (power-of-two) f32s that hit ftoa.ts's one-digit boundary fixup
    // (exp_bin -119 / 64 / 67): 2^-96, 2^87, 2^90.
    2 ** -96,
    2 ** 87,
    2 ** 90,
];

function header(fn, mod = "../dtoa") {
    return [
...
const fLines = header("ftoa", "../ftoa");
// dtoa-comp: the compressed-table f64 path. Same dtoa source as dtoa.spec.ts -
// the compressed pow10 anchor table + reconPow10 is selected at build time when
// this spec is compiled with --shrinkLevel >= 1 (the os-* / oz-* test modes), so
// these suites exercise the reconstruction path. Uses a fresh random f64 set so
// it adds coverage rather than re-testing dtoa.spec's exact inputs.
const dcLines = [
    `// AUTO-GENERATED by scripts/dtoa/gen-spec.mjs - do not edit by hand.`,
...
const emitDC = (title, vals) =>
    emit(dcLines, "dtoa", litD, refDouble, title, vals);

const powD = Array.from({ length: 633 }, (_, i) =>
    Number(`1e${i - 323}`),
).filter(Number.isFinite);

emitD("dtoa: f64 edge cases", edgeD);
emitD("dtoa: powers of ten", powD);
// random f64
const randD = [];
...
// dtoa-comp: edges + powers of ten + a *fresh* random f64 set (drawn after the
// dtoa/ftoa sets, so different values) - additive compressed-path coverage.
const randDC = [];
for (let i = 0; randDC.length < N; i++) {
    const v = f64from((BigInt(r32()) << 32n) | BigInt(r32()));
    if (Number.isFinite(v)) randDC.push(v);
}
emitDC("dtoa-comp: f64 edge cases", edgeD);
emitDC("dtoa-comp: powers of ten", powD);
emitDC("dtoa-comp: random f64", randDC);
...
write("dtoa-comp.spec.ts", dcLines);
