...
// assembly/__benches__ twin of benches/dtoa-comp.bench.ts; same series, relative imports.

import { bench, dumpToFile, blackbox } from "./lib/bench";
import { dtoa, dtoa_buffered } from "../dtoa";
import { dtoa as zmijDtoa, dtoa_buffered as zmijDtoaBuffered } from "../../vendor/zmij-as/assembly/dtoa";
import {
  dtoa as stdlibDtoa,
...
const ZERO_SPECIAL: f64[] = [0, -0, Infinity, -Infinity, NaN, 0, -0, Infinity];
const SMALL_INTEGERS: f64[] = [1, -7, 42, -99, 100, -999, 1000, -9999];
const LARGE_INTEGERS: f64[] = [10000, -123456, 1000000, -12345678, 99999999, -100000000, 999999999, -1000000000];
const TINY_FIXED: f64[] = [1, -1, 0.5, -0.5, 10, 100, 1000, 1e-6];
...
function benchZmijNoalloc(): void {
  for (let i = 0, n = current.length; i < n; i++) {
    blackbox<u32>(zmijDtoaBuffered(U16, unchecked(current[i])));
  }
}
